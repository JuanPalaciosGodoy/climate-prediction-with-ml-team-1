...
def get_storms_summary_data(storms: xarray.Dataset) -> pd.DataFrame:
    ...
    year = np.array(storms.avg_year.values).astype(int)
    month = np.array(storms.avg_month.values).astype(int)
...
def storms_kmeans(
    storms: xarray.Dataset, k_clusters: int, get_variables
) -> xarray.Dataset:
    """
    ...
    cluster_xarray = _assign_cluster(
        storms=storms,
...
def kmeans_inertia(storms: xarray.Dataset, get_variables, max_clusters: int = 15):
    ...
    plot_kmeans_inertia(
        sum_of_squares=sum_of_squares, max_clusters=range(1, max_clusters)
    )


def storms_dbscan(
    storms: xarray.Dataset, eps: float, min_samples: int, get_variables
) -> xarray.Dataset:
    """
...
def storms_gmm(
    storms: xarray.Dataset, n_clusters: int, get_variables
) -> xarray.Dataset:
    """
...
def get_storms_moments(storms: xarray.Dataset) -> np.array:
    ...
    try:
        ...
        return np.array(
            [
                storms.lon_mean.values[mask],
                storms.lat_mean.values[mask],
                storms.lon_var.values[mask],
                storms.lat_var.values[mask],
                storms.lon_lat_cov.values[mask],
            ],
            storms.pdi.values[mask],
        ).T
    except:
        raise ValueError(
            "need to calculate moments first by using the function get_storms_with_moments()"
        )


def filter_storms(
    storms: xarray.Dataset,
    years: list = [],
    storm_ids: list = [],
    kmeans_clusters: list = [],
    dbscan_clusters: list = [],
) -> xarray.Dataset:
    """
...
def get_storms_with_moments(
    storms: xarray.Dataset, weights: list = []
) -> xarray.Dataset:
    """
    ...
    moments_list = np.array(
        [
            _get_storm_moments(storms.sel(storm=i), weights=weights)
            for i in range(storms.dims["storm"])
            if _get_storm_moments(storms.sel(storm=i), weights=weights)
        ]
    ).T
    ...
    # moments
    lon_mean_xarray = _assign_cluster(
        storms=storms, cluster_labels=lon_mean, name="lon_mean", data_type="float"
    )
    lat_mean_xarray = _assign_cluster(
        storms=storms, cluster_labels=lat_mean, name="lat_mean", data_type="float"
    )
    lon_var_xarray = _assign_cluster(
        storms=storms, cluster_labels=lon_var, name="lat_var", data_type="float"
    )
    lat_var_xarray = _assign_cluster(
        storms=storms, cluster_labels=lat_var, name="lat_var", data_type="float"
    )
    lon_lat_cov_xarray = _assign_cluster(
        storms=storms, cluster_labels=lon_lat_cov, name="lon_lat_cov", data_type="float"
    )
    lon_std_xarray = _assign_cluster(
        storms=storms, cluster_labels=lon_std, name="lon_std", data_type="float"
    )
    lat_std_xarray = _assign_cluster(
        storms=storms, cluster_labels=lat_std, name="lat_std", data_type="float"
    )
    lon_lat_corr_xarray = _assign_cluster(
        storms=storms,
        cluster_labels=lon_lat_corr,
        name="lon_lat_corr",
        data_type="float",
    )

    # deltas
    lon_delta_xarray = _assign_cluster(
        storms=storms, cluster_labels=lon_delta, name="lon_delta", data_type="float"
    )
    lat_delta_xarray = _assign_cluster(
        storms=storms, cluster_labels=lat_delta, name="lat_delta", data_type="float"
    )

    # dates
    month_xarray = _assign_cluster(
        storms=storms, cluster_labels=avg_month, name="avg_month", data_type="int"
    )
    year_xarray = _assign_cluster(
        storms=storms, cluster_labels=avg_year, name="avg_year", data_type="int"
    )
    day_xarray = _assign_cluster(
        storms=storms, cluster_labels=avg_day, name="avg_day", data_type="int"
    )

    # pdi
    pdi_xarray = _assign_cluster(
        storms=storms, cluster_labels=pdi, name="pdi", data_type="float"
    )

    # lifespan
    lifespan_xarray = _assign_cluster(
        storms=storms, cluster_labels=lifespan, name="lifespan(Days)", data_type="float"
    )
...
def _get_storm_moments(storm: xarray.Dataset, weights: list = []) -> tuple[list]:
    ...
    if len(weights) != len(lon_lst):
        raise ValueError(
            f"length of weights <{len(weights)}> must be the same size as the length of the lon and lat of the storm <{len(lon_lst)}>"
        )
    else:
        # apply weights
    ...
    timestamps = pd.to_datetime(
        [t.decode("utf-8") if isinstance(t, bytes) else t for t in timestamps],
        errors="coerce",
    )
...
def _assign_cluster(
    storms: xarray.Dataset, cluster_labels: list, name: str, data_type: str
) -> xarray.DataArray:
    """
    ...
    complete_clusters = _complete_cluster_list(
        storms=storms, cluster_labels=cluster_labels
    )
